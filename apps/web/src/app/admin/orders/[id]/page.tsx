...
export default async function AdminOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-8 max-w-3xl  text-brand-800">
      <h1 className="text-2xl font-bold text-brand-800">Order {order.id.slice(0, 8)}…</h1>
          ...
          <tbody>
            {order.items.map((item: AdminOrderItem) => (
              <tr key={item.id} className="border-b border-surface-100">
                ...
                <td className="py-2 text-right font-semibold">{formatMoney(item.total)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="text-right font-bold text-brand-800 mt-4">Total: {formatMoney(order.total)}</p>
      </section>

      {/* Status updater */}
      <AdminStatusUpdater orderId={id} currentStatus={order.status} currentTracking={order.trackingNumber ?? ''} />
    </div>
  );
}